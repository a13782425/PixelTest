...
public class RaySampleManager : MonoBehaviour
{
    ...
    IEnumerator StartExport(int totalFrames)
    {
        ...
        //Animation animation = modelRoot.GetComponentInChildren<Animation>();
        ////animation.Stop();
        //var state = animation["swim"];
        //state.layer = 0;
        //Debug.LogError(state.name);
        //var length = animation.clip.length;
        //for (int i = 0; i < totalFrames; i++)
        //{
        //    state.normalizedTime = i / (float)totalFrames;
        //    animation.Sample();
